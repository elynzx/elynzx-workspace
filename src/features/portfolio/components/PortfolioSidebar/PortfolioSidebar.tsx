...
export const PortfolioSidebar = ({
  ...
  return (
    <div
      className={`w-full border-b md:border-b-0 md:border-r border-white/50 p-3 md:p-4 grid grid-cols-4 md:flex md:flex-col gap-2 shrink-0 transition-all duration-300 ${containerSizing}`}
    >
      <div
        className={`hidden md:block text-[11px] text-white uppercase tracking-widest font-black px-2.5 mb-2 truncate transition-opacity duration-300 ${headerVisibility}`}
      >
        Navigation
      </div>
      ...
      {NAVIGATION.map(({ id, label, Icon }) => {
        ...
        const tabButtonStyles = isTabActive
          ? "bg-pink-400 text-white border-white/20 shadow-md scale-[1.02]"
          : "bg-white/15 border-transparent text-white/80 hover:bg-white/[0.15] hover:text-white hover:scale-[1.01]";
